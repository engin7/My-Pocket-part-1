...
@objc public class AppDatabase : NSObject {
    /// The shared AppDatabase
    @objc static var shared: AppDatabase!
...
extension AppDatabase {
    ...
    func savePlayer(_ player: inout Entries) throws {
    ...
    func deletePlayers(ids: [Int64]) throws {
        try dbQueue.write { db in
            _ = try Entries.deleteAll(db, keys: ids)
        }
    }
...
//        try dbQueue.write { db in
//            if try Entries.fetchCount(db) == 0 {
//                // Insert new random players
//                try createRandomPlayers(db)
//            } else {
//                // Insert a player
//                if Bool.random() {
//                    var player = Entries.newRandom()
//                    try player.insert(db)
//                }
//                // Delete a random player
//                if Bool.random() {
//                    try Entries.order(sql: "RANDOM()").limit(1).deleteAll(db)
//                }
//                // Update some players
//                for var player in try Entries.fetchAll(db) where Bool.random() {
//                    try player.updateChanges(db) {
//                        $0.score = Entries.randomScore()
//                    }
//                }
//            }
//        }
